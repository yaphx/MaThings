...
var intents = new builder.IntentDialog({ recognizers: [recognizer] })
    .matches('Conversation.Greeting', (session, args) => {
        varrand = Math.floor(Math.random() * (generalReplies.length));
    ...
    .matches('BusTiming.NextBus', (session, args) => {
        ...
        if (busStopNum) {
            ...
            request({
                ...
                if (error) {
                    returnconsole.log('Error:', error);
                }

                if (response.statusCode !== 200) {
                    returnconsole.log('Invalid Status Code Returned:', response.statusCode);
                }
                varobj = JSON.parse(body);
                varbusStopNum = obj.BusStopID;
                varservices = obj.Services;

                varservice = services[0];
                if (service != null) {
                    ...
                    if (inOp === "In Operation") {
                        ...
                        if (estArr) {
                            ...
                            var milli = Math.abs(newDate(estArr) - newDate());
                        ...
                        if (subEstArr) {
                            ...
                            var subMilli = Math.abs(newDate(subEstArr) - newDate());
